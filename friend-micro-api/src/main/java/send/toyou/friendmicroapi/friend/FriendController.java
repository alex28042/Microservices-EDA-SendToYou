...
public class FriendController {
    ...
    public Mono<ResponseEntity<Friend>> saveFriend(@RequestBody Friend friend){
        return this.friendService.addNewFriend(friend)
    ...
    public Mono<ResponseEntity<Friend>> getPackage(@RequestParam String name){
        return this.friendService.getFriendByName(name).doOnNext(friend -> log.info("Friend found out: {}", friend))
                .map(friend -> friend != null
                        ? new ResponseEntity<>(friend, HttpStatus.OK)
